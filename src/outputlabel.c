...
GtkWidget *shape_create_icon(char *xpm_file,
			     gint x,
			     gint y, gint px, gint py, gint window_type);
#endif
...
static void outputlabel_init(Outputlabel * a)
{
}
...
GtkWidget *outputlabel_new(module * module, int output_i)
{
	...
	gtk_frame_set_shadow_type(GTK_FRAME(outputlabel),
	...
	outputlabel->timeout_tag = gtk_timeout_add(30,
						   (GtkFunction)
						   outputlabel_update_levelmeters,
						   outputlabel);
			     ...
			     gint y, gint px, gint py, gint window_type)
{
	...
	gdk_pixmap =
	    gdk_pixmap_create_from_xpm(window->window, &gdk_pixmap_mask,
...
int channel_calc_level(channel * ch, int offset)
{
	gint i;
	long long rtot = 0;
...
gint outputlabel_update_levelmeters(gpointer data)
{
	...
	if (outputlabel->module->on) {
		levelmeter_set_level(LEVELMETER
				     (outputlabel->levelmeter_left),
				     channel_calc_level(outputlabel->
							module->
							outputs
							[outputlabel->
							 output_i], 0));
		levelmeter_set_level(LEVELMETER
				     (outputlabel->levelmeter_right),
				     channel_calc_level(outputlabel->
							module->
							outputs
							[outputlabel->
		...
		levelmeter_set_level(LEVELMETER
				     (outputlabel->levelmeter_left), 0);
		levelmeter_set_level(LEVELMETER
				     (outputlabel->levelmeter_right), 0);
	}
