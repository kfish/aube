...
static void levelmeter_size_request(GtkWidget * widget,
				    GtkRequisition * requisition);
static void levelmeter_size_allocate(GtkWidget * widget,
...
GtkWidget *levelmeter_new(guint level)
{
...
static gint levelmeter_expose(GtkWidget * widget, GdkEventExpose * event)
{
	...
	levelmeter_height =
	    widget->allocation.height / LEVELMETER_DEFAULT_DIVISIONS;
	...
	gdk_gc_set_foreground(levelmeter_style->fg_gc[GTK_STATE_NORMAL],
			      &col_green);

	for (i = 0;
	     i < LEVELMETER_DEFAULT_HIGH && i <= LEVELMETER(widget)->level;
	     i++) {
		gdk_draw_rectangle(widget->window,
				   levelmeter_style->fg_gc[widget->state],
				   TRUE, 0,
				   (LEVELMETER_DEFAULT_DIVISIONS -
				    i) * levelmeter_height,
				   widget->allocation.width,
				   levelmeter_height - 1);
	}
	...
	for (;
	     i <= LEVELMETER_DEFAULT_DIVISIONS
	     && i <= LEVELMETER(widget)->level; i++) {
		gdk_draw_rectangle(widget->window,
				   levelmeter_style->fg_gc[widget->state],
				   TRUE, 0,
				   (LEVELMETER_DEFAULT_DIVISIONS -
