...
minitracker *minitracker_new()
{
	...
	snprintf(s->module.outputs[0]->u_label,
		 sizeof(s->module.outputs[0]->u_label), "seq");
...
void seq_toggle(minitracker * rsq)
{
	rsq->seq_on = 1 - rsq->seq_on;
}

void seq_restart(minitracker * rsq)
{
	rsq->seqi = 0;
}

void shift_left(minitracker * rsq)
{
	rsq->seqi += SEQ_LENGTH;
	rsq->seqi--;
	rsq->seqi %= SEQ_LENGTH;
...
void shift_right(minitracker * rsq)
{
	rsq->seqi++;
	rsq->seqi %= SEQ_LENGTH;
}

void chaos(minitracker * rsq)
{
	int i;

	for (i = 0; i < SEQ_LENGTH; i++) {
		if ((i == 0 || rsq->sequence[i - 1].note)
		    && ((rand() % SEQ_LENGTH) < SEQ_SILENCE)) {
...
void seq_chaos8(minitracker * rsq)
{
	int i;
...
void seq_play_44(minitracker * rsq)
{
	int i;
...
void seq_transpose_u12(minitracker * rsq)
{
	seq_transpose(rsq, 12);
}

void seq_transpose_d12(minitracker * rsq)
{
	seq_transpose(rsq, -12);
}

void seq_transpose_u1(minitracker * rsq)
{
	seq_transpose(rsq, 1);
}

void seq_transpose_d1(minitracker * rsq)
{
	seq_transpose(rsq, -1);
}

void seq_transpose(minitracker * rsq, int delta)
{
	int i;
#if 0
	int nri;
