...
sample_recorder *sample_recorder_new()
{
	...
	snprintf(sr->module.u_label, sizeof(sr->module.u_label),
		 "s-recorder");
	snprintf(sr->module.outputs[0]->u_label,
		 sizeof(sr->module.outputs[0]->u_label), "out");
...
int sample_recorder_calc_note(sample_recorder * sr)
{
	...
	if (sr->record_next) {
		seq_ch =
		    (seq_channel *) sr->module.inputs[1]->channel->data;
	...
	if (sr->recording) {
		n = MIN(tick,
			AUBE_SR_BUFFER_TICKS * DEFAULT_TICK -
			sr->rec_offset);
		n *= 2;
	...
	case AUBE_SAMPLE_RECORDER_PASSTHROUGH_REC:
		if (sr->recording)
			memcpy(sr->tick_buffer,
			       ((bit16 *) sr->module.inputs[0]->channel->
				data), tick * 2);
		else
			sample_recorder_play_buffer(sr);
		break;
	case AUBE_SAMPLE_RECORDER_PASSTHROUGH_ALWAYS:
		memcpy(sr->tick_buffer,
		       ((bit16 *) sr->module.inputs[0]->channel->data),
		       tick * 2);
		break;
	default:
		break;
