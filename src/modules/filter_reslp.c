...
int filter_reslp_calc_note(filter_reslp * filter_reslp)
{
	...
	if (filter_reslp->use_trigger == 1
	    && filter_reslp->module.inputs[1]->channel != NULL) {

		seq_ch =
		    (seq_channel *) filter_reslp->module.inputs[1]->
		    channel->data;
		...
		for (j = 0; j < ENVELOPE_POINTS_PER_TICK; j++) {
			if (filter_reslp->env_i >= 0) {
				...
				filter_reslp->resonance =
				    (double) filter_reslp->resonance_base *
				    ((double) filter_reslp->
				     resonance_envelope[filter_reslp->
							env_i]) / 50.0;
				filter_reslp->cutoff =
				    (double) filter_reslp->cutoff_base *
				    ((double) filter_reslp->
				     cutoff_envelope[filter_reslp->
						     env_i]) / 50.0;
		...
		for (i = 0; i < tick; i++) {
			filter_reslp->tick_buffer[i] =
			    (bit16)
			    iir_filter((float)
				       (((bit16 *) filter_reslp->module.
					 inputs[0]->channel->data)[i]),
				       filter_reslp->iir);
...
void calc_filter(filter_reslp * filter_reslp)
{
	reslp_calc(filter_reslp->iir, (double) filter_reslp->resonance,
		   (double) filter_reslp->cutoff,
		   (double) filter_reslp->gain / 100.0);
}
