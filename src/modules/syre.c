...
int 
syre_if_calc_note(syre_if_data * syre_if)
{
  unsigned int i, j, k;
  int spq;			/*
				   samples per tick 
  ...
  if (syre_if->module.inputs[0]->channel != NULL) {
    ...
    seq_ch = (seq_channel *) syre_if->module.inputs[0]->channel->data;
	...
	if (seq_ch->accent) {
	  amp_env_val = (int) (((int) syre_if->amp_envelope_accented[syre_if->env_i / TICKS_PER_ENVELOPE_POINT] * seq_ch->vol) >> 6);
	  pitch_env_val = syre_if->pitch_envelope_accented[syre_if->env_i / TICKS_PER_ENVELOPE_POINT];
	} else {
	  amp_env_val = (int) (((int) syre_if->amp_envelope[syre_if->env_i / TICKS_PER_ENVELOPE_POINT] * seq_ch->vol) >> 6);
	  pitch_env_val = syre_if->pitch_envelope[syre_if->env_i / TICKS_PER_ENVELOPE_POINT];
	}
	...
	for (k = 0; k < spq; k += 2) {
	  ...
	  if (syre_if->bi < syre_if->port_sub &&
	      syre_if->sequence[(syre_if->seqi + SEQ_LENGTH - 1) % SEQ_LENGTH].note &&
	      syre_if->sequence[syre_if->seqi].portamento_toggle) {
	    pitch = (syre_if->port_val * syre_if->sequence[syre_if->seqi].note +
		     (32768 - syre_if->port_val) *
		     syre_if->sequence[(syre_if->seqi + SEQ_LENGTH - 1) % SEQ_LENGTH].note) >> 15;
	    syre_if->port_val += syre_if->port_delta;
	  } else {
	    pitch = syre_if->sequence[seqi].note;
	  }
	  ...
	  wip = ((int) syre_if->wi + syre_if->waveform_length - pitch / 2) % syre_if->waveform_length;
	  win = ((int) syre_if->wi + pitch / 2) % syre_if->waveform_length;

	  syre_if->tick_buffer[i] =
	    (bit16) (((syre_if->waveform[(int) syre_if->wi]) * amp_env_val) >> 6);
#if 0
	  (bit16) ((((syre_if->waveform[(int) syre_if->wi] + (syre_if->waveform[wip] + syre_if->waveform[win]) / 2) / 2) * amp_env_val) >> 6);
#endif

	  syre_if->tick_buffer[i + 1] =
	    (bit16) ((syre_if->tick_buffer[i] * rpan) >> 4);
	  syre_if->tick_buffer[i] =
	    (bit16) ((syre_if->tick_buffer[i] * lpan) >> 4);



	  syre_if->wi += pitch;
	  if (syre_if->wi > (float) syre_if->waveform_length)
	    syre_if->wi = syre_if->wi - (float) syre_if->waveform_length;

	  i += 2;
	}
      } else {
	for (k = 0; k < spq; k += 2) {
	  syre_if->tick_buffer[i] = (bit16) 0;
	  syre_if->tick_buffer[i + 1] = (bit16) 0;
	  i += 2;
	}
      }
      ...
      if (syre_if->env_i > -1) {
	syre_if->env_i++;
	if (syre_if->env_i > (ENVELOPE_LEN * TICKS_PER_ENVELOPE_POINT))
	  syre_if->env_i = -1;
      }
    }				/*
				   j 
...
void 
calc_waveform(syre_if_data * syre_if)
{
  int i, j;
  bit32 d;
/*
 ...
 */
  ...
  for (j = 0; j < NUM_HARMONICS; j++) {
    ...
    env_i = (syre_if->sequence[syre_if->seqi].note * syre_if->overtones[j].r) >> 12;
  ...
  for (i = 0; i < syre_if->waveform_length; i++) {
    ...
    for (j = 0; j < NUM_HARMONICS; j++) {
       ...
       */
      d += (syre_if->current_wave[syre_if->overtones[j].si] *
	    (syre_if->overtones[j].vol - 16));
      syre_if->overtones[j].si += syre_if->overtones[j].r;
...
void 
chaos_harmonics(syre_if_data * syre_if)
{
  int i;
  ...
  for (i = 0; i < NUM_HARMONICS; i++) {
    ...
    syre_if->overtones[i].vol = rand() % (32 - (i * 16 / NUM_HARMONICS));
  }
  syre_if->harmonics_changed = 1;
}

void 
clear_harmonics(syre_if_data * syre_if)
{
  int i;
...
void 
clear_odd_harmonics(syre_if_data * syre_if)
{
  int i;
