...
GtkWidget *syre_if_new(syre_if_data * mod)
{
	...
	gtk_curve_set_range(GTK_CURVE
			    (GTK_GAMMA_CURVE(syre_if->amp_env_curve)->
			     curve), 0, ENVELOPE_LEN, 0, 64);
	gtk_curve_set_vector(GTK_CURVE
			     (GTK_GAMMA_CURVE(syre_if->amp_env_curve)->
			      curve), ENVELOPE_LEN,
			     syre_if->data->amp_envelope);
	/*
	...
	gtk_curve_set_range(GTK_CURVE
			    (GTK_GAMMA_CURVE(syre_if->pitch_env_curve)->
			     curve), 0, ENVELOPE_LEN, 0, 64);
	gtk_curve_set_vector(GTK_CURVE
			     (GTK_GAMMA_CURVE(syre_if->pitch_env_curve)->
			      curve), ENVELOPE_LEN,
			     syre_if->data->pitch_envelope);
	/*
	...
	gtk_curve_set_range(GTK_CURVE
			    (GTK_GAMMA_CURVE
			     (syre_if->amp_env_curve_accented)->curve), 0,
			    ENVELOPE_LEN, 0, 64);
	gtk_curve_set_vector(GTK_CURVE
			     (GTK_GAMMA_CURVE
			      (syre_if->amp_env_curve_accented)->curve),
			     ENVELOPE_LEN,
			     syre_if->data->amp_envelope_accented);
	/*
	...
	gtk_curve_set_vector(GTK_CURVE
			     (GTK_GAMMA_CURVE
			      (syre_if->pitch_env_curve_accented)->curve),
			     ENVELOPE_LEN,
			     syre_if->data->pitch_envelope_accented);
	/*
	...
	for (i = 0; i < NUM_HARMONICS; i++) {
		...
		syre_if->sliders[i] =
		    slider_int_new(nc, &(syre_if->data->overtones[i].vol),
				   0, 32, 1);
	...
	gtk_curve_set_range(GTK_CURVE
			    (GTK_GAMMA_CURVE(syre_if->freq_env_curve)->
			     curve), 0, ENVELOPE_LEN, 0, 64);
	gtk_curve_set_vector(GTK_CURVE
			     (GTK_GAMMA_CURVE(syre_if->freq_env_curve)->
			      curve), ENVELOPE_LEN,
			     syre_if->data->freq_envelope);
	/*
...
void copy_accented_cb(GtkWidget * widget, gpointer data)
{
	...
	gtk_curve_get_vector(GTK_CURVE
			     (GTK_GAMMA_CURVE
			      (SYRE_IF(data)->amp_env_curve)->curve),
			     ENVELOPE_LEN,
			     SYRE_IF(data)->data->amp_envelope);
	gtk_curve_set_vector(GTK_CURVE
			     (GTK_GAMMA_CURVE(syre_if->pitch_env_curve)->
			      curve), ENVELOPE_LEN,
			     syre_if->data->pitch_envelope_accented);
	gtk_curve_get_vector(GTK_CURVE
			     (GTK_GAMMA_CURVE
			      (SYRE_IF(data)->pitch_env_curve)->curve),
			     ENVELOPE_LEN,
			     SYRE_IF(data)->data->pitch_envelope);
}
...
void copy_unaccented_cb(GtkWidget * widget, gpointer data)
{
	...
	gtk_curve_get_vector(GTK_CURVE
			     (GTK_GAMMA_CURVE
			      (SYRE_IF(data)->amp_env_curve_accented)->
			      curve), ENVELOPE_LEN,
			     SYRE_IF(data)->data->amp_envelope_accented);
	gtk_curve_set_vector(GTK_CURVE
			     (GTK_GAMMA_CURVE
			      (syre_if->pitch_env_curve_accented)->curve),
	...
	gtk_curve_get_vector(GTK_CURVE
			     (GTK_GAMMA_CURVE
			      (SYRE_IF(data)->pitch_env_curve_accented)->
			      curve), ENVELOPE_LEN,
			     SYRE_IF(data)->data->pitch_envelope_accented);
}
...
gint get_envelopes(gpointer data)
{
	...
	if ((((module *) data)->on)
	    && (((syre_if_data *) (SYRE_IF(data)->data))->env_i <=
		ENVELOPE_POINTS_PER_TICK)) {

		gtk_curve_get_vector(GTK_CURVE
				     (GTK_GAMMA_CURVE
				      (SYRE_IF(data)->amp_env_curve)->
				      curve), ENVELOPE_LEN,
				     SYRE_IF(data)->data->amp_envelope);
		gtk_curve_get_vector(GTK_CURVE
				     (GTK_GAMMA_CURVE
				      (SYRE_IF(data)->pitch_env_curve)->
				      curve), ENVELOPE_LEN,
				     SYRE_IF(data)->data->pitch_envelope);
		gtk_curve_get_vector(GTK_CURVE
				     (GTK_GAMMA_CURVE
				      (SYRE_IF(data)->
				       amp_env_curve_accented)->curve),
				     ENVELOPE_LEN,
				     SYRE_IF(data)->data->
		...
		gtk_curve_get_vector(GTK_CURVE
				     (GTK_GAMMA_CURVE
				      (SYRE_IF(data)->
				       pitch_env_curve_accented)->curve),
				     ENVELOPE_LEN,
				     SYRE_IF(data)->data->
		...
		gtk_curve_get_vector(GTK_CURVE
				     (GTK_GAMMA_CURVE
				      (SYRE_IF(data)->freq_env_curve)->
				      curve), ENVELOPE_LEN, tmp_env);
		for (i = 0; i < ENVELOPE_LEN; i++) {
			if (tmp_env[i] !=
			    SYRE_IF(data)->data->freq_envelope[i]) {
				SYRE_IF(data)->data->harmonics_changed = 1;
				break;
			}
		}
		if (SYRE_IF(data)->data->harmonics_changed) {
			memcpy(SYRE_IF(data)->data->freq_envelope, tmp_env,
			       ENVELOPE_LEN * sizeof(gfloat));
		}
#endif
		...
		gtk_curve_get_vector(GTK_CURVE
				     (GTK_GAMMA_CURVE
				      (SYRE_IF(data)->cutoff_env_curve)->
				      curve), ENVELOPE_LEN,
				     SYRE_IF(data)->data->cutoff_envelope);
#endif
	}
	return 1;
}
