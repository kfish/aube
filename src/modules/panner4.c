...
panner4 *panner4_new()
{
	...
	snprintf(pn->module.u_label, sizeof(pn->module.u_label),
		 "panner4");
	snprintf(pn->module.outputs[0]->u_label,
		 sizeof(pn->module.outputs[0]->u_label), "fwd");
	pn->module.outputs[0]->module = (module *) pn;
	snprintf(pn->module.outputs[1]->u_label,
		 sizeof(pn->module.outputs[1]->u_label), "rear");
...
int panner4_calc_note(panner4 * pn)
{
	...
	for (i = 0; i < tick; i += 2) {
		l = (bit16) ((((bit16 *) pn->module.inputs[0]->channel->
			       data)[i] * lpan) >> 4);
		r = (bit16) ((((bit16 *) pn->module.inputs[0]->channel->
			       data)[i + 1] * rpan) >> 4);
