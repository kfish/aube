...
static void master_class_init(MasterClass * klass)
{
...
static void aube_refresh_samplelist(GtkWidget * widget, gpointer * data)
{
	int i;
	gchar *title[3];
	gchar buf1[16];
	...
	for (i = 0; i < aube_daddy->nr_samples; i++) {
		...
		snprintf(title[2], sizeof(title[2]), "%d",
			 aube_daddy->samples[i]->parms.d.audio_sample_data.
			 s_length);
...
static gint
aube_samplelist_events(GtkWidget * widget, GdkEventButton * bevent,
		       gpointer data)
{
	GtkWidget *clist;
	gint row, col;
	...
	if (bevent->button == 3) {
		gtk_clist_get_selection_info(GTK_CLIST(clist), bevent->x,
					     bevent->y, &row, &col);
...
static void aube_remove_sample_cb(GtkWidget * widget, gpointer data)
{
	aube_remove_sample(selected_sample);
}

static void master_init(Master * master)
{
	...
	for (i = 0; i < LAST_UNIT_TYPE; i++) {
		menuitem =
		    gtk_menu_item_new_with_label((char *)
	...
	for (i = 0; i < (sizeof(module_classes) / sizeof(module_class *));
	     i++) {
		menuitem =
		    gtk_menu_item_new_with_label(module_classes[i]->
						 u_name);
		gtk_menu_append(GTK_MENU
				(module_type_menus
				 [module_classes[i]->category]), menuitem);
		g_signal_connect(G_OBJECT(menuitem), "activate",
				 G_CALLBACK(new_module_cb),
				 module_classes[i]);
		gtk_widget_show(menuitem);
	}
#endif
	...
	menu2 =
	    modulemenu_new(NULL, (void *) include_unhidden,
			   (void *) select_unhidden);
	gtk_menu_item_set_submenu(GTK_MENU_ITEM(menuitem), menu2);
	gtk_menu_append(GTK_MENU(menu), menuitem);
	...
	im = gtk_image_new_from_file(g_strconcat
				     (DATADIR, "/aube_mini.png", NULL));
	...
	for (i = 0; i < (sizeof(module_classes) / sizeof(module_class *));
	     i++) {
		button =
		    gtk_button_new_with_label(module_classes[i]->u_name);
	...
	g_signal_connect(G_OBJECT(master),
			 "modules_changed",
			 G_CALLBACK(aube_refresh_samplelist), clist);
	...
	for (i = 0; i < aube_daddy->nr_modules; i++) {
		for (j = 0; j < aube_daddy->modules[i]->nr_inputs; i++) {
			for (k = 0; k < ou->nr_outputs; k++) {
				if (aube_daddy->modules[i]->inputs[j]->
				    channel == ou->outputs[k]) {
					aube_daddy->modules[i]->inputs[j]->
					    channel = u->outputs[k];
					l++;
				}
			}
...
void change_replace_menu_cb(GtkWidget * widget, gpointer data)
{
	...
	menu =
	    modulemenu_new(NULL, (void *) include_unhidden,
			   (void *) select_unhidden);
	menu =
	    modulemenu_new((module *) data, (void *) aube_module_cmp_type,
...
void clear_workspace_cb(GtkWidget * widget, gpointer data)
{
	...
	for (i = 0; i < n; i++) {
		if ((mc = aube_daddy->modules[j]->class)
		    && strcmp(mc->type_label, "none")) {
